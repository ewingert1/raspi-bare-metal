...
#define OMGREKT_SPEED 	(3)

#define CPT_VALUE		(0x3F000)




#define GPIO_ADDRESS 			0x3F200000
#define ITR_CTRL_REGS_ADDRESS	0x3F00B200

#define 	IRQ_Basic_Pending_Reg_Address		(ITR_CTRL_REGS_ADDRESS)
#define 	IRQ_Pending1_Reg_Address			(ITR_CTRL_REGS_ADDRESS+0x04)
#define 	IRQ_Pending2_Reg_Address			(ITR_CTRL_REGS_ADDRESS+0x08)
#define 	FIQ_Control_Reg_Address				(ITR_CTRL_REGS_ADDRESS+0x0C)
#define 	Enable_IRQs1_Reg_Address			(ITR_CTRL_REGS_ADDRESS+0x10)
#define 	Enable_IRQs2_Reg_Address			(ITR_CTRL_REGS_ADDRESS+0x14)
#define 	Enable_Basic_IRQs_Reg_Address		(ITR_CTRL_REGS_ADDRESS+0x18)
#define 	Disable_IRQs1_Reg_Address			(ITR_CTRL_REGS_ADDRESS+0x1C)
#define 	Disable_IRQs2_Reg_Address			(ITR_CTRL_REGS_ADDRESS+0x20)
#define 	Disable_Basic_IRQs_Reg_Address		(ITR_CTRL_REGS_ADDRESS+0x24)
...
//	if(*((uint32_t*)IRQ_Pending2_Reg_Address) & (0xF<<17))
//	{
//		if(ETAT){
//			SET_GPIO_PIN_HIGH(26);
//		}
...
void set_pin_level(unsigned int gpio_n, unsigned int level)
{
	if (level)
	{
		gpio_add[0x1C/4+(gpio_n/32)] = (1 << (gpio_n%32) );	//Set the SET Reg
	}
	else
	{
		gpio_add[0x28/4+(gpio_n/32)] = (1 << (gpio_n%32) );	//Set the CLEAR Reg
	}
}
...
void setupIO()
{
	...
	SET_GPIO_PIN_OUT(24);
//	SET_GPIO_PIN_IN(5);
//	//set pull up on gpio5
//	set_pin_pull_Res(5,2);
//	SET_GPIO_PIN_ASYNC_RISING_EDGE(5);
//	//Clear events
//	gpio_add[0x40/4]=1<<5;
}
...
int  notmain()
{
	//initInterrupt();
	unsigned int i = 0;
	unsigned int PROFILE=NORMAL_SPEED;


	setupIO();
	//initInterrupt();
	SET_GPIO_PIN_LOW(26);
	//set_pin_level(26,ETAT);

	//while(!(PIN_HAS_EVENT(5)));
	while(1)
	{
		//set_pin_level(26,1);
		SET_GPIO_PIN_HIGH(26);
		for(i=0;i<(CPT_VALUE/(PROFILE+1));i++);
		SET_GPIO_PIN_HIGH(23);
		for(i=0;i<(CPT_VALUE/(PROFILE+1));i++);
		SET_GPIO_PIN_HIGH(19);
		for(i=0;i<(CPT_VALUE/(PROFILE+1));i++);
		SET_GPIO_PIN_HIGH(24);
		for(i=0;i<(CPT_VALUE/(PROFILE+1));i++);
		//set_pin_level(26,0);
		SET_GPIO_PIN_LOW(24);
		for(i=0;i<(CPT_VALUE/(PROFILE+1));i++);
